...
o	Build time Dependency
o	Run-Time Dependencies
...
3.	Poetry - A Python dependency manager to handle project dependencies.
4.	Liquibase - A database change management tool that helps manage schema changes in a controlled manner.
Purpose
...
Redis:
Redis (REmote DIctionary Server) is an open source, in-memory, NoSQL key/value store that is used primarily as an application cache or quick-response database. It stores data in memory, rather than on a disk or solid-state drive (SSD), which helps deliver unparalleled speed, reliability, and performance.
Why Redis:
...
Use causes:
•	Caching Attendance Data: Redis stores frequently accessed records in memory, reducing database queries and enhancing response times.
